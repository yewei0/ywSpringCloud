$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'hdb/orderattachment/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.orderAttachment.id == null ? "hdb/orderattachment/save" : "hdb/orderattachment/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.orderAttachment),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "hdb/orderattachment/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "hdb/orderattachment/info/"+id, function(r){
                vm.orderAttachment = r.orderAttachment;
            });
		},
